...
test('真实账本：资源、收支、转账、调账、退款、作废和持久化', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: '统计对比摘要' }).getByText('普通收入', { exact: true })).toBeVisible()
  const categoryRow = page.locator('[data-statistics-section="category"] tr').filter({ hasText: expense }).first()
  await expect(categoryRow).toContainText('¥70.00')
  await categoryRow.getByRole('button', { name: '查看明细', exact: true }).click()
  await expect(page.getByRole('dialog')).toContainText('净支出')
  await expect(page.getByRole('dialog')).toContainText('¥70.00')
  await page.getByRole('button', { name: '关闭统计明细', exact: true }).click()
