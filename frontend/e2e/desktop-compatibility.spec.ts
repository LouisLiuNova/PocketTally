...
test.describe('Chromium 桌面兼容性矩阵', () => {
  ...
  test('常规数据、长文本、分页、统计和弹窗保持可用', async ({ page }) => {
    ...
    const walletCard = page.locator('[data-account-id]').filter({ hasText: fixture.wallet.name })
    ...
    await walletCard.getByRole('button', { name: `更多操作：${fixture.wallet.name}`, exact: true }).click()
