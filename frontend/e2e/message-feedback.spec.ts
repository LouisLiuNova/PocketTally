...
test('设置页可以手动审阅四级反馈消息', async ({ page }) => {
  ...
  const alertWidth = await alerts.evaluate(element => element.getBoundingClientRect().width)
  const cardWidth = await page.locator('.settings-card').last().evaluate(element => element.getBoundingClientRect().width)
  expect(cardWidth).toBeCloseTo(alertWidth, 0)
