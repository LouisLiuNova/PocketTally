...
test('分类和标签支持 HEX/RGB 双向选色、字段校验与编辑回填', async ({ page }) => {
  ...
  const categoryRow = page.locator('[data-category-row-id]').filter({ has: page.getByText(category, { exact: true }) })
  await categoryRow.getByRole('button', { name: `更多操作：${category}`, exact: true }).click()
  ...
  const tagRow = page.locator('[data-tag-id]').filter({ has: page.getByText(tag, { exact: true }) })
  ...
  await page.getByRole('tab', { name: /标签/ }).click()
  const persistedTagRow = page.locator('[data-tag-id]').filter({ has: page.getByText(tag, { exact: true }) })
  await expect(persistedTagRow.locator('.tag-color-dot')).toHaveCSS('background-color', 'rgb(171, 205, 239)')
  await expect(persistedTagRow).toContainText(tag)
  await persistedTagRow.getByRole('button', { name: '删除', exact: true }).click()
  await expect(page.getByRole('button', { name: '取消', exact: true })).toBeFocused()
  await page.getByRole('button', { name: '确认操作', exact: true }).click()
  await expect(persistedTagRow).toHaveCount(0)
})
