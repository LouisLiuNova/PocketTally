  ...
  <UApp>
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <nav aria-label="主导航"><button v-for="item in navItems" :key="item.label" type="button" class="nav-item" :class="{ active: activeView === item.label }" :disabled="!loaded" :aria-label="item.label" :aria-current="activeView === item.label ? 'page' : undefined" @click="activeView = item.label"><UIcon :name="item.icon" /><span class="nav-label">{{ item.label }}</span></button></nav>
        ...
        <template v-if="loaded">
          ...
          <template v-if="activeView === '总览'">
            ...
            <div class="dashboard-grid">
              <section class="panel"><div class="panel-head"><h2>现金流趋势</h2><span class="hint">退款按到账日计入流入</span></div><p v-if="!cashFlow?.buckets.length" class="empty-state">本期暂无现金流</p><div v-else class="real-trend" tabindex="0" aria-label="现金流趋势数据"><div v-for="item in cashFlow.buckets" :key="item.startAt" class="trend-row"><span>{{ bucketLabel(item.startAt).slice(5) }}</span><div><div class="trend-track"><i :style="{ width: `${item.incomeAmountMinor / trendMax * 100}%` }" /></div><div class="trend-track expense-track"><i :style="{ width: `${item.expenseAmountMinor / trendMax * 100}%` }" /></div></div><small>收入 {{ money(item.incomeAmountMinor) }} · 退款 {{ money(item.refundAmountMinor) }} · 支出 {{ money(item.expenseAmountMinor) }} · 净额 {{ money(item.netCashFlowMinor) }}</small></div></div></section>
    ...
    <UModal :open="!!transactionEditor" :dismissible="true" title="交易表单" @update:open="value => { if (!value) transactionEditor = null }"><template #content><TransactionEditor v-if="transactionEditor" v-bind="transactionEditor" :accounts="accounts" :categories="categories" :tags="tags" :refund-summary="refundSummary" @close="transactionEditor = null" @saved="saved" /></template></UModal>
    <UModal :open="!!resourceEditor" :dismissible="true" title="资源表单" @update:open="value => { if (!value) resourceEditor = null }"><template #content><ResourceEditor v-if="resourceEditor" v-bind="resourceEditor" :categories="categories" @close="resourceEditor = null" @saved="saved" /></template></UModal>
