...
认证模型位于 `auth.*.schema.json`：登录和密码修改请求保留密码原文语义，服务端只保存
Argon2id 哈希；`SessionRead`/`SessionList` 只返回会话时间、设备摘要和当前标记，不返回
Cookie 或令牌。认证库结构见 `docs/contracts/db-auth.dbml`。
