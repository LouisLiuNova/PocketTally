...
本文是 PocketTally 的通用前端测试方案和执行指示，记录前端基础检查、真实后端 E2E、桌面兼容性矩阵、失败排查和交付判定标准。Issue #20 的 Chromium 桌面验收是当前方案的完整示例。
...
Issue #20 只有在以下条件全部满足时才算通过：

1. Chromium 1280、1440、1920 三档矩阵全部通过。
2. 五个主页面没有横向溢出、关键控件遮挡或不可操作问题。
...
8. 前端类型检查、单元测试、生产构建、E2E、后端测试、Ruff、文档检查和 `git diff --check` 全部通过。
