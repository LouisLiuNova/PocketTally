...
首次使用：创建账户 → 使用调账录入现有余额 → 创建收入/支出分类 → 记账。前端支持资源增删改、交易搜索筛选、收支/转账/调账、编辑、退款和作废；统计按浏览器本地日期归属月份，退款抵减支出，转账和调账不参与收支统计。
...
当前适用于单用户本地运行，未加入登录鉴权。交易列表由 API 一次读取后在前端筛选与分页，适合当前小型账本 MVP；大量数据和导入功能留待后续。
