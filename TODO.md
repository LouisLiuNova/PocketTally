...
- [ ] 完成分类用途决策 `DEC-CAT-001`：确定是否严格区分 `income` 与 `expense`，以及父子分类是否必须用途一致；随后同步 HTTP Schema、ORM、数据库约束和报表口径。
- [ ] 完成退款、冲正和交易修订规则：落实退款关联、关系循环、累计金额和原路退回约束，并关闭 `DEC-REF-001`、`DEC-REF-002`、`DEC-REV-001` 与 `DEC-LIF-001` 中影响本任务的决策。
- [x] 确定余额以有效交易为权威来源：新建账户余额固定为 0，不设置初始余额字段；余额调整、交易修改、作废和转账在同一事务中同步可重建余额，并保留审计记录。
...
- [ ] 完成 `DEC-LIF-001`：定义交易生命周期和时间语义，区分发生、录入、入账、修改、作废与冲正时间，并明确各状态允许修改的字段。
- [ ] 完成 `DEC-RPT-001`：定义拆分交易、分类树聚合和退款抵减后的报表归属；转账不得计入收入或支出。

## MVP 后续功能

- [ ] 完成 `DEC-AUTH-001` 后增加用户鉴权、资源所有权与数据隔离；该功能不属于当前 MVP 范围。
